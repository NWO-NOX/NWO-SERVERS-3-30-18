...
_uid = "76561197974176270"; // Needs to be a valid UID that exists in the account table (best to use a server owners uid)
...
[
	...
	// Choppers
	["Exile_Chopper_Hellcat_Green",1,[],true],		
	["Exile_Chopper_Hummingbird_Green",1,[],true],
	["Exile_Chopper_Orca_CSAT",1,[],true]
];
		...
		if !(_count >= _x select 1) then
		{
			...
			else
			{
				...
				waitUntil 
				{
					...
					_min = 300; 																				// minimum distance from the center position (Number) in meters
					_max = 40000; 																			// maximum distance from the center position (Number) in meters
					_mindist = 500; 																			// minimum distance from the nearest object (Number) in meters, ie. spawn at least this distance away from anything within x meters..
					...
					_onRoadCheck = _startPosRoad nearRoads 500; 											//Find road objects 100m from spot
